...
# Read Data (NetCDF4)
inp1 = Dataset('/content/drive/MyDrive/input/CMIP5.input.36mon.1861_2001.nc','r')
inp2 = Dataset('/content/drive/MyDrive/input/CMIP5.label.12mon.1863_2003.nc','r')
...
#测试集soda100
soda = Dataset('/content/drive/MyDrive/input/SODA.input.36mon.1871_1970.nc','r')
lablesoda = Dataset('/content/drive/MyDrive/input/SODA.label.12mon.1873_1972.nc','r')
...
def main():
    ...
    for epoch in range(epochs):
        ...
        cor.astype('float32').tofile('/content/drive/MyDrive/cmip/lmontcmipcor.gdat')
        
        if not math.isnan(thiscor):
            if thiscor >= bestcor:
                torch.save(copy.deepcopy(model.state_dict()), '/content/drive/MyDrive/cmip/bestcmip_model.pth')#只保存权重,state_dict只是浅拷贝
            ...
            torch.save(model.state_dict(), '/content/drive/MyDrive/cmip/last_model.pth')
        else:
            if train_loss < bestloss:

                torch.save(copy.deepcopy(model.state_dict()), '/content/drive/MyDrive/cmip/bestcmip_model.pth')
                bestloss = train_loss
